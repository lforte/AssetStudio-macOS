...
namespace AssetStudio
{
    ...
    public class SerializedProgram
    {
        ...
        public SerializedProgram(ObjectReader reader)
        {
            ...
            if (version[0] > 2022 || (version[0] == 2022 && version[1] >= 1)) //2022.1 and up - player build data separation
            {
                int numPlayerSubProgramGroups = reader.ReadInt32();
                m_PlayerSubPrograms = new SerializedPlayerSubProgram[numPlayerSubProgramGroups][];
                for (int i = 0; i < numPlayerSubProgramGroups; i++)
                {
                    int numPlayerSubPrograms = reader.ReadInt32();
                    m_PlayerSubPrograms[i] = new SerializedPlayerSubProgram[numPlayerSubPrograms];
                    for (int j = 0; j < numPlayerSubPrograms; j++)
                    {
